...
def test_create_and_fill_bucket(make_stubber, make_unique_name, monkeypatch):
    ...
    bucket, stanza_objects = batch_versioning.create_and_fill_bucket(
...
def test_create_and_fill_bucket_failure(make_stubber, make_unique_name, monkeypatch):
    ...
    s3_stubber.stub_put_object(
        bucket_name, f"{obj_prefix}stanza-0", error_code="TestException"
    )
...
def test_prepare_for_cleanup(make_stubber, make_unique_name):
    ...
    manifest_lines = batch_versioning.prepare_for_cleanup(
        batch_versioning.s3.Bucket(bucket_name),
        obj_prefix,
...
def test_setup_demo(monkeypatch):
    ...
    role, bucket, stanza_objects = batch_versioning.setup_demo(
        role_name, "bucket", function_info, "test-prefix"
    )
    assert role.name == role_name
...
def test_usage_demo_batch_operations(
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        ...
        runner.add(
            ...
            bucket.name,
            prefix=stanza_prefix,
            object_keys=[stanza_key],
            keep_going=(stop_index != 1),
        )
        runner.add(
            s3_stubber.stub_list_objects,
            bucket.name,
...
def test_teardown_demo(
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        ...
        runner.add(
            s3_stubber.stub_delete_object_versions,
            bucket_name,
            versions,
            keep_going=True,
        )
